<template>
    <div>
        <el-input :placeholder="placeholder" :size="size" @compositionstart.native="onzhinputend"
                  ...
                  ref="input2" :spellcheck="false" v-model="currentDesc"  v-bind="$attrs"
                  v-on="$listeners" @clear="clear">
...
export default {
  ...
  methods: {
    ...
    opencodedialog (e) {
      if (this.$refs.input2.disabled) return
      ...
      handler (val) {
        if (this.currentValue !== val) {
          ...
          if (!this.autoTranslate) {
            this.currentDesc = this.description
            return
          }
