<template>
    <d2-container>
          <el-form :inline="true" @keyup.enter.native="getDataList()" size="small">
             <el-form-item>
                 <el-input clearable placeholder="姓名" v-model="dataForm.like_name"></el-input>
             </el-form-item>
             <el-form-item>
             ...
             <el-form-item>
                   <el-button @click="addOrUpdateHandle()" type="primary" v-if="$hasPermission('demo:teststudentx:save')">增加</el-button>
             </el-form-item>
             <el-form-item>
                   <el-button @click="deleteHandle()" type="danger" v-if="$hasPermission('demo:teststudentx:delete')">批量删除</el-button>
             </el-form-item>
             <el-form-item>
                   <el-button @click="exportHandle()" type="success" v-if="$hasPermission('demo:teststudentx:export')">导出</el-button>
             </el-form-item>
            </el-form>
            <el-table
                    ref="listTable"
            ...
            <el-table-column align="center"  header-align="center" label="操作" width="120">
                    <template slot-scope="scope">
                        <el-button @click="addOrUpdateHandle(scope.row.stdId)" size="small" type="text"
                                   v-if="$hasPermission('demo:teststudentx:update')">修改</el-button>
                        <el-button @click="deleteHandle(scope.row.stdId)" size="small" type="text"
                                   v-if="$hasPermission('demo:teststudentx:delete')">删除</el-button>
                    </template>
                </el-table-column>
            </el-table>
...
import addOrUpdate from './teststudentx-add-or-update'
export default {
  name: 'demo-teststudentx',
  mixins: [resizemix],
  components: {
    addOrUpdate
  },
  ...
  methods: {
    ...
    getDataList () {
      // 测试取回表中所有列
      // console.info(this.$refs.tableData.$slots.default[3].componentOptions.propsData)
      const params = {
        order: this.order,
