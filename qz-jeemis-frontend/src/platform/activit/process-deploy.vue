<template>
  <qz-dialog :visible.sync="visible" :title="$t('process.deployFile')" :close-on-click-modal="false" :close-on-press-escape="false">
    <el-upload
      name="processFile"
      :action="url"
      :file-list="fileList"
...
export default {
  ...
  methods: {
    init () {
      this.visible = true
      this.url = `${window.SITE_CONFIG.apiURL}/act/process/deploy?token=${util.cookies.get('token')}`
      this.fileList = []
    },
    // 上传之前
